...
#' deprecated version of makeBayouModel
.makeBayouModel_deprecated <- function(f, rjpars, cache, prior, impute=NULL, startpar=NULL, moves=NULL, control.weights=NULL, D=NULL, shiftpars=c("sb", "loc", "t2"), model="OU"){
  vars <- terms(f)
  ...
  if(length(rjpars) > 0){
    if(is.null(moves)){
      moves =  c(switch(model, "OU" = list(alpha=".multiplierProposal", sig2=".multiplierProposal"), 
                 ...
                 c(theta=".vectorSlidingWindowSplit", k=".splitmergebd", slide=".slide2"))
    } 
