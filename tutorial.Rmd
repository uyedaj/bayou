...
mcmcOU <- bayou.makeMCMC(tree, dat, SE=MEvar, prior=priorOU, 
                        outname="modelOU_r001", plot.freq=NULL) # Set up the MCMC
...
```{r}
quantiles <- c(0.05, 0.1, 0.25, 0.5, 0.75, 0.975)
...
```{r}
library(doParallel)
registerDoParallel(cores=detectCores() - 1) #Using more cores will make it go faster if you have them.
